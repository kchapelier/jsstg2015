...
module.exports = function doubleRotatorFactory (rng, speed, generosity, difficulty) {
    ...
    for (var i = 0; i < 60; i++) {
        def.push(
            ['wait', waitTime],
            ['burst', bulletNumber, Math.PI * 2,  Math.PI * Math.sin((i >> d) / 10), false],
            ['burst', bulletNumber, Math.PI * 2,  Math.PI * 2 * Math.sin((i >> d) / 10), false],
            ['burst', bulletNumber, Math.PI * 2,  Math.PI * 4 / 3 * Math.sin((i >> d) / 10), false]
        );
    }
